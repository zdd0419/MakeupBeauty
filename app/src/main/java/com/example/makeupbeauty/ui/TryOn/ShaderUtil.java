...
public final class ShaderUtil {
     ...
     */
    public static int loadGLShader(Context context, int type, String filename, Map<String, Integer> defineValuesMap){
        // Load shader source code.
        String code = null;
        try {
            code = readShaderFileFromAssets(context, filename);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        if (compileStatus[0] == 0) {
            Log.e("GL", "Error compiling shader: " + GLES20.glGetShaderInfoLog(shader));
    ...
    public static void loadTexture(Context context, int[] textureId, String filename){
        Bitmap textureBitmap = null;
        try {
            textureBitmap = BitmapFactory.decodeStream(context.getAssets().open(filename));
        } catch (IOException e) {
            e.printStackTrace();
        }
